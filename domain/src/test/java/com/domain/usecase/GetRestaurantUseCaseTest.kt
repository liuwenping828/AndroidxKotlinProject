...
import com.domain.repository.restaurant.usecase.GetRestaurantUseCase
...
class GetRestaurantUseCaseTest {
    ...
    fun `returns the success result when success retrieving the restaurant`() {
        ...
        sut.execute(1).test().assertValueAt(1) { (it as Success).restaurants == restaurant }
    }
    ...
    fun `returns the failure result when error retrieving the restaurant`() {
        ...
        sut.execute(1).test().assertValueAt(1) { (it as Failure).throwable == throwable }
    }
